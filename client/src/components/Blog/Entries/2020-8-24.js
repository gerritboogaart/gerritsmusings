...
export default (
  <React.Fragment>
    ...
    Even though I love to use Lodash' filter, map, and reduce, sometimes in life you have no choice and you find yourself forced to use vanilla JS.  And let's face it: working in pure JavaScript is awesome.
    <hr />
    JavaScript has a couple of crazy powerful native functions, three of my favorites are map, reduce and filter.<br />
    The difference between 'map' and 'reduce' is that map will always return an array of the length of the array you are sending. [1,2,3].map(n => n + 1).length will return 3. Even if you'd say [1,2,3].map(n => undefined).length; it will still be 3. It will be [undefined, undefined, undefined]
